...
const services = [
  { icon: Building2, title: "Business registration", description: "Thoughtful support at the point your enterprise takes shape.", items: ["Private limited companies", "Limited liability partnerships", "One person companies", "Section 8 companies", "Foreign subsidiaries"], href: "/company-incorporation", cta: "Explore incorporation" },
  { icon: Landmark, title: "Filings & compliance", description: "A reliable rhythm for essential statutory responsibilities.", items: ["GST registration", "Annual ROC & LLP filings", "DIR-3 KYC", "Ongoing company retainership"], href: "/ra-practice", cta: "Explore RA practice" },
  { icon: FileSignature, title: "Contracts & drafting", description: "Clear paperwork that reflects the understanding behind a deal.", items: ["Founder & shareholder agreements", "Employment & vendor agreements", "Partnership & franchise agreements", "NDAs & lease documentation"], href: "/services", cta: "Explore capabilities" },
  { icon: BadgeCheck, title: "Licences & registrations", description: "Practical help navigating registrations needed for operations.", items: ["IEC & MSME (Udyam)", "Startup India registration", "Shop & Establishment licence", "12A & 80G registration"], href: "/ra-practice", cta: "Explore RA practice" },
  { icon: Scale, title: "SEBI practice support", description: "Process-led assistance for regulated investment professionals.", items: ["Research analyst registration", "Investment adviser registration", "One-time compliance setup", "RA & IA audit readiness"], href: "/ra-practice", cta: "Explore RA practice" },
  { icon: BriefcaseBusiness, title: "Strategic assistance", description: "A considered legal perspective when your business is moving forward.", items: ["FEMA reporting: FCGPR & FCTRS", "Funding-readiness guidance", "Board meeting assistance", "Company conversions & secretarial work"], href: "/practice-areas", cta: "Explore practice areas" },
];
...
export default function Home() {
  ...
  return <main id="home">
    ...
    <section className="service-section" id="services"><div className="shell"><div className="section-heading"><div><p className="eyebrow">OUR PRACTICE AREAS</p><h2>Services shaped<br />around your <em>business.</em></h2></div><p>From incorporation through everyday compliance and specialised regulatory work, our support is precise, practical and personal.</p></div><div className="service-grid">{services.map(({ icon: Icon, title, description, items, href, cta }, index) => <article className="service-card" key={title}><div className="service-top"><span>0{index + 1}</span><span className="service-icon"><Icon size={38} strokeWidth={1.35} /></span></div><h3>{title}</h3><p>{description}</p><ul>{items.map((item) => <li key={item}><Check size={13} strokeWidth={1.8} />{item}</li>)}</ul><a className="service-card-cta" href={href}>{cta} <ArrowDownRight size={16} /></a></article>)}</div></div></section>
